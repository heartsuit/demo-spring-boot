...
public class WeatherDaoImpl implements WeatherDao {
    ...
    public int add(Weather weather) {
        return jdbcTemplate.update(
                "insert into jdbc_template.weather(ts, temperature, humidity) VALUES(?,?,?)",
    ...
    public int[] batchInsert(List<Weather> weatherList) {
        return jdbcTemplate.batchUpdate("insert into jdbc_template.weather(ts, temperature, humidity) values( ?, ?, ?)", new BatchPreparedStatementSetter() {
            @Override
    ...
    public List<Weather> queryForList(int limit, int offset) {
        return jdbcTemplate.query("select * from jdbc_template.weather limit ? offset ?", (rs, rowNum) -> {
    ...
    @Override
    public int count() {
        return jdbcTemplate.queryForObject("select count(*) from jdbc_template.weather", Integer.class);
    }
}
